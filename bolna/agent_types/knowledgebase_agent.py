...
Use this information naturally when it helps answer the user's questions. Don't force references if not relevant to the conversation."""
    ...
    async def generate(
    ...
    ) -> AsyncGenerator[Union[Tuple[str, bool, Optional[Dict], bool, None, None], Dict], None]:
        """
        ...
        try:
            ...
            if metadata.get("latency"):
                meta_info["rag_latency"] = {"sequence_id": meta_info.get("sequence_id"), **metadata["latency"]}
